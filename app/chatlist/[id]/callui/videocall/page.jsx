...
//         // 2️⃣ Initialize peer connection
...
//             if (e.candidate) {
//                 socket.current.emit("webrtc-candidate", { roomId: ROOM_ID, candidate: e.candidate });
//             }
//         };

//         // 5️⃣ Socket events
//         socket.current.on("webrtc-offer", async ({ sdp }) => {
//             await pc.current.setRemoteDescription({ type: "offer", sdp });
//             const answer = await pc.current.createAnswer();
//             await pc.current.setLocalDescription(answer);
//             socket.current.emit("webrtc-answer", { roomId: ROOM_ID, sdp: answer.sdp });
//         });

//         socket.current.on("webrtc-answer", async ({ sdp }) => {
...
//         // 6️⃣ Join room and create offer
//         socket.current.emit("join-room", { roomId: ROOM_ID });

//         const timer = setTimeout(async () => {
//             const offer = await pc.current.createOffer();
//             await pc.current.setLocalDescription(offer);
//             socket.current.emit("webrtc-offer", { roomId: ROOM_ID, sdp: offer.sdp });
//         }, 1000);
...
//             />
//             <video
//                 style={{ width: 200, height: 150, position: "absolute", right: 20, top: 20 }}
//                 autoPlay
//                 muted
//                 playsInline
